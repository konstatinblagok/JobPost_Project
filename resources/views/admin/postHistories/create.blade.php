...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.postHistory.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.post-histories.store") }}" enctype="multipart/form-data">
            @csrf
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('job_post') ? 'is-invalid' : '' }}" name="job_post_id" id="job_post_id" required>
                    @foreach($job_posts as $id => $job_post)
                        <option value="{{ $id }}" {{ old('job_post_id') == $id ? 'selected' : '' }}>{{ $job_post }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('post_location') ? 'is-invalid' : '' }}" name="post_location_id" id="post_location_id" required>
                    @foreach($post_locations as $id => $post_location)
                        <option value="{{ $id }}" {{ old('post_location_id') == $id ? 'selected' : '' }}>{{ $post_location }}</option>
                    @endforeach
                </select>
